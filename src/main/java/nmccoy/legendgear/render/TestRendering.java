...
public class TestRendering {
	...
	public static void drawFancyReticle(EntityPlayer player, Vec3 pos, double radius, float progress, boolean tint)
	{
		...
		if(!player.isUsingItem())
		{
			...
			if(tint)GL11.glColor4ub((byte)255,(byte)255,(byte)128, (byte)128);
			drawHorizontalRing(pos.xCoord, pos.yCoord, pos.zCoord, radius, 8);
			GL11.glDepthFunc(GL11.GL_GREATER);
			GL11.glColor4ub((byte)255,(byte)255,(byte)255, (byte)32);
			if(tint)GL11.glColor4ub((byte)255,(byte)255,(byte)128, (byte)32);
			drawHorizontalRing(pos.xCoord, pos.yCoord, pos.zCoord, radius, 8);
		}
		else if(progress < 1)
		{
	...
	public void renderWorldLastEvent(RenderWorldLastEvent event)
	{
		...
		{
			SpellItem spell = (SpellItem) player.getHeldItem().getItem();
		...
		//	System.out.println("Look:"+playerLook);
			...
			Vec3 alsoFrom = Vec3.createVectorHelper(playerEye.xCoord, playerEye.yCoord, playerEye.zCoord);
			...
			if(targetPos.distanceTo(alsoFrom) < castRange) rayhit = true;
			//System.out.println(rayhit);
...
/*
			...
			drawHorizontalRing(targetPos.xCoord, targetPos.yCoord, targetPos.zCoord, castRadius, 32);
			*/
			drawFancyReticle(player, targetPos, castRadius, spell.getCastingProgress(weapon, player, event.partialTicks), rayhit);
