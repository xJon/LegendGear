...
//	}
	...
	public void handleCollect(EntityItemPickupEvent eipe)
	{
		...
		if(LegendGear.quiverAllowed && item.itemID == Item.arrow.shiftedIndex)
		{
		...
		if(LegendGear.allowBombs && item.itemID == LegendGear.bombItem.shiftedIndex)
		{
		...
		if(LegendGear.heartsAllowed && item.itemID == LegendGear.heartPickup.shiftedIndex)
		{
	...
	public ItemStack findDamaged(EntityPlayer p, Item type)
	{
		...
		{
			if(inv[i] != null && inv[i].itemID == type.shiftedIndex && inv[i].getItemDamage() > 0)
				return inv[i];
		}
	...
	public void handleHurt(LivingHurtEvent lhe)
	{
				...
				!lhe.entityLiving.worldObj.isRemote)
		{
			...
			if(lhe.source.damageType.equals("fall"))
			{
				...
				if(ep.isUsingItem() && is != null && is.itemID == LegendGear.geoAmulet.shiftedIndex)
				{
	...
	public void handleFireBow(ArrowLooseEvent ale)
	{
		...
		ItemStack quiv = findNonemptyQuiver(ale.entityPlayer);
		if(quiv != null)
		{
            ...
            if (!ale.entityPlayer.worldObj.isRemote)
            {
            	...
            	if(LegendGear.enableBombArrows && ale.entityPlayer.isSneaking() && ale.entityPlayer.inventory.hasItem(LegendGear.bombItem.shiftedIndex))
            	{
            		ale.entityPlayer.inventory.consumeInventoryItem(LegendGear.bombItem.shiftedIndex);
	...
	private ItemStack findNonfullBombBag(EntityPlayer player)
	{
		...
		{
			if(inv[i] != null && inv[i].itemID == LegendGear.bombBag.shiftedIndex && inv[i].getItemDamage() < LegendGear.maxBombBagCapacity)
			{
				suitableBag = inv[i];
				break;
			}
	...
	private ItemStack findNonfullQuiver(EntityPlayer player)
	{
		...
		for(i = 0; i < inv.length; i++)
		{
			if(inv[i] != null && inv[i].itemID == LegendGear.quiver.shiftedIndex && inv[i].getItemDamage() < LegendGear.maxQuiverCapacity)
			{
				moreArrows = inv[i];
				break;
			}
	...
	private ItemStack findNonemptyQuiver(EntityPlayer player)
	{
		ItemStack[] inv = player.inventory.mainInventory;
		ItemStack moreArrows = null;
		int i;
		for(i = 0; i < inv.length; i++)
		{
			if(inv[i] != null && inv[i].itemID == LegendGear.quiver.shiftedIndex && inv[i].getItemDamage() > 0)
			{
				moreArrows = inv[i];
				break;
			}
	...
	public void phoenixEffect(LivingDeathEvent lde)
	{
		if(LegendGear.allowPFeather && lde.entityLiving instanceof EntityPlayer /*&& lde.entityLiving.getHealth() <= lde.entityLiving.lde.ammount*/)
		{
			...
			if(ep.inventory.hasItem(LegendGear.phoenixFeather.shiftedIndex))
			{
				if(!ep.worldObj.isRemote)
				{
					ep.inventory.consumeInventoryItem(LegendGear.phoenixFeather.shiftedIndex);
	...
	public void shrubHandler(PlayerInteractEvent pie)
	{
		...
		if(LegendGear.shrubSuperPrizes &&
			...
			world.getBlockId(pie.x, pie.y, pie.z) == LegendGear.mysticShrubID &&
			...
			world.canLightningStrikeAt(pie.x, pie.y, pie.z))
		{
			...
			case 7:
				...
				break;
			case 8:
				fabulousPrizes = new ItemStack(Item.record13.shiftedIndex + world.rand.nextInt(Item.recordWait.shiftedIndex - Item.record13.shiftedIndex + 1), 1, 0);
				break;
			case 9:
	...
	public ArrayList<ItemStack> getLootFrom(EntityLiving el)
	{
				...
				(el instanceof EntityCreeper && ((EntityCreeper)el).getPowered()))
		{
			...
			case 5: case 6: case 7:
				...
				{
					items.add(new ItemStack(LegendGear.emeraldShard, 1, 0));
				} break;
			case 8: 
				if(LegendGear.emeraldShardsAllowed) 
				{
					items.add(new ItemStack(LegendGear.emeraldShard, 3, 0));
				} break;	
			case 9:
				if(LegendGear.emeraldShardsAllowed) 
				{
					items.add(new ItemStack(LegendGear.emeraldShard, 1, 1));
				} break;
			default:
			}	
		}
	...
	public void bonusLoot(LivingDropsEvent lde)
	{
		...
		if(!world.isRemote && lde.recentlyHit && !(lde.entityLiving instanceof EntityPlayer))
		{
			int xp = lde.entityLiving.experienceValue;
			EntityLiving ent = lde.entityLiving;
			
			for(ItemStack i:getLootFrom(ent))
			{
